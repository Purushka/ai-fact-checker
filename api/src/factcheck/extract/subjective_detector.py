...
SUBJECTIVE_KEYWORDS = [
    "最适合", "最好的", "最佳", "最差", "最强", "更有前景", "更值得",
    "应该选", "建议选", "推荐", "可能成为", "可能会", "有望",
    "前途光明", "前景广阔", "潜力", "倾向于",
]
...
class SubjectiveDetector:
    ...
    async def is_subjective(self, claim: str, token_acc: TokenAccumulator) -> tuple[bool, str]:
        ...
        resp = await self.provider.chat(
            ...
            temperature=0.0, max_tokens=120, timeout=15.0,
        )
        token_acc.add_llm(
            provider=resp.provider, model=resp.model,
            prompt_tokens=resp.prompt_tokens, completion_tokens=resp.completion_tokens,
            total_tokens=resp.total_tokens, label="subjective_detector",
        )
