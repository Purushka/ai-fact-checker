...
| 形态 | 位置 | 用途 |
|---|---|---|
| **Claude Skill** | `.claude/skills/fact-check/` | Claude 直接调用，用于验证流程、跑测试集 |
...
- `.claude/skills/fact-check/SKILL.md`：skill 入口（产品定义）
- `.claude/skills/fact-check/pipeline.md`：7 步流程
- `.claude/skills/fact-check/scoring.md`：评分公式 + gating rules
- `.claude/skills/fact-check/reports/run_initial_executed.json`：10 个 case 实测结果
